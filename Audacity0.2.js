...
function newLevel(){
	clientMessage("\n" + ChatColor.GRAY + "[AD]" + split + ChatColor.RED + NAME + " Version:" + split + VERSION);
...
function mainMenu(){
	ctx.runOnUiThread(new Runnable({ run: function(){
		try{
			...
			var FEnc = new Button(ctx);
			FEnc.setText("ForceEnc (Indev)");
			FEnc.setTextColor(Color.BLUE);
			FEnc.setOnClickListener(new View.OnClickListener({
      	  onClick: function(viewarg){
        		Player.enchant(Player.getSelectedSlotId(), Enchantment.UNBREAKING,10);
             Player.enchant(Player.getSelectedSlotId(), Enchantment.KNOCKBACK,2);
            Player.enchant(Player.getSelectedSlotId(), Enchantment.LOOTING,2);
             Player.enchant(Player.getSelectedSlotId(), Enchantment.SHARPNESS,10);
            Player.enchant(Player.getSelectedSlotId(), Enchantment.FLAME,1);
             Player.enchant(Player.getSelectedSlotId(), Enchantment.FORTUNE,3);
            Player.enchant(Player.getSelectedSlotId(), Enchantment.FIRE_ASPECT,1);
           Player.enchant(Player.getSelectedSlotId(), Enchantment.DEPTH_STRIDER,5);
           ...
           Player.enchant(Player.getSelectedSlotId(), Enchantment.SMITE,5);
			...
			menuLayout.addView(FEnc);
			
			gm = new CheckBox (ctx);
			gm.setText ("ToggleGM");
			gm.setTextColor(Color.RED);
			gm.setChecked(gmch);
			gm.setOnClickListener(new View.OnClickListener({
				onClick: function(viewarg){
					if(!gmch){
						gmch = true;
					gm.setTextColor(Color.GREEN)
					Level.setGameMode(1);
				}else{
					gmch = false;
					gm.setTextColor(Color.RED);
			...
			menuLayout.addView(gm);
			
			if(gmch) {
			   gm.setTextColor(Color.GREEN);
			         }
			speedmine = new CheckBox(ctx);
			speedmine.setText("Speedbreak");
			...
			kill.setText("Kill Aura (Only works on SP)");
			...
			invplace.setText("Infinite Place Items");
			...
			effect.setText("Effects (Only works on SP)");
			...
			maker.setText("MODIFIED by" + split + DEVELOPERS);
			maker.setTextColor(Color.RED);
			maker.setGravity(Gravity.CENTER);
			menuLayout.addView(maker);
			
			menu = new PopupWindow(menuLayout1, ctx.getWindowManager().getDefaultDisplay().getWidth()/3.5, ctx.getWindowManager().getDefaultDisplay().getHeight());
